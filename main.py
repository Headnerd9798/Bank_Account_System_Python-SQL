...
def get_cursor(cursor, isolation_level="SERIALIZABLE"):
    ...
    if cursor is None:
        ...
        cnx = get_new_connection(default_db_host, default_db_user, default_db_password)
        cursor = cnx.cursor()
...
def create_account(balance, cursor):
    ...
    try:
        ...
        q = "insert into w4111f19final.banking_account (balance, version) values(%s, %s)"

        #sql, args = None, fetch = True, cur = None, conn = final_conn, commit = True, debug = False)
        result = run_q(q, args=(balance, version), fetch=True, commit=False, cur=cursor)

        # By definition, we are in a transaction. So, auto-increment must be the largest value for
        # the auto-increment field.
        q = "select max(id) as new_id from w4111f19final.banking_account;"
        result = run_q(q, args=None, fetch=True, commit=False, cur=cursor)
...
def get_balance(id, cursor=None):
    ...
    try:
        ...
        q = "select * from w4111f19final.banking_account where id=%s"
        result = run_q(q, args=id, fetch=True, commit=False, cur=cursor)
...
def update_balance(id, amount, cursor=None):
    ...
    try:
        ...
        q = "update w4111f19final.banking_account set balance=%s, version=%s where id=%s"
        result = run_q(q, args=(amount, new_version, id), fetch=True, commit=False, cur=cursor)
...
def update_balance_optimistic(acct, amount, cursor=None):
    ...
    try:
        ...
        result = run_q(q, args=(amount, new_version, acct['id']), fetch=True, commit=False, cur=cursor)
